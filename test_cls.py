...
from Networks.models import base_patch16_384_token, base_patch16_384_gap, base_patch16_384_attention, \
    base_patch16_384_fgap, base_patch16_384_swin, base_patch16_384_effnet, base_patch16_384_cls
...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, gt_count) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            mask = (out2 > 0.0).float()
            count = torch.sum(out1* mask).item()
        if i>10:
    ...
    print(' \n throughput: {}'.format(1/throughput.avg))
